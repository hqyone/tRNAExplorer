...
def tRNA_DB_Preparing(db_name, tRNA_bed, ref_fasta, tRNAscanSE, offset=60, no_mit_tRNA=True, no_pseudogenes=True, min_qscore=30):
    ...
    result = CreatetRNAFastas(
        tRNA_bed, db_name, ref_fasta, trna_db_dir, offset)
    ...
    output_code = RuntRNAScan(trna_db_dir, tRNAscanSE,
                              trna_utr_fasta, trna_scan_out)
    if output_code != -1:
        ...
        if parsetRNAScanFile(trna_scan_out,
            ...
            print("###### tRNA database named \'"+db_name +
...
def main(argv):
    ...
    min_qscore = 30
    try:
        opts, args = getopt.getopt(argv, "n:b:r:s:u:", [
                                   "name=", "bed=", "ref=", "scan=", "offset=", "no_mit=", "no_pseu=", "minq="])
    ...
    for opt, arg in opts:
        if opt == '-h':
            ...
            print('# -b <bed> : Absolute path of bed file for tRNAs')
            print(
                '# -r <ref> : Absolute path of Genome FASTA file coordinating with bed file')
            print('# -s <tRNAScanSE> : Absolute path of tRNAscan SE program')
            print('# -u <offset> : The length of UTRs')
            print(
                '# --no_mit <1/else> : 1 means removing all nucleic mitochondrial tRNA genes')
            print('# --no_pseu <1/else> : 1 means removing all pseudogene tRNA genes')
            print('# --minq <number> : The minimum of quality scores of tRNAs')
            print(
                '# Output 1: annotation bed file of tRNAs <Directory of tRNA bed file>/<name>.bed')
            print(
                '# Output 2: FASTA file of hybrid tRNA library <Directory of tRNA bed file>/<name>.fa')
            sys.exit(0)
        elif opt in ("-n", "--name"):
            db_name = arg
            ...
            min_qscore = int(arg)
    if os.path.isfile(tRNAscanSE):
        tRNA_DB_Preparing(db_name, tRNA_bed, ref_fasta, tRNAscanSE, offset=offset,
                          no_mit_tRNA=no_mit_tRNA, no_pseudogenes=no_pseudogenes, min_qscore=min_qscore)
        exit(0)
    else:

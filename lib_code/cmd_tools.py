    ...
    def executeCMD(self, cmd):
        try:
            ...
            if process.returncode == 0:
                ...
                print('Command excutation failed with return {}'.format(
                    process.returncode))
                return -1
        except:
...
class BLASTN(CMDTool):
    ...
    def alignment(self, db_fasta, query_fasta, eval, blast_out_file, hit_number=30, num_threads=4, *argv, **kwrgs):
        if os.path.isfile(db_fasta) and os.path.isfile(query_fasta):
            cmd = self.getCMD('-task blastn -db {} -query {} -evalue {} -num_threads {} {} -num_alignments {} > {}'.format(
                db_fasta, query_fasta, eval, num_threads, self.output_format_str, hit_number, blast_out_file),  *argv, **kwrgs)
            # print(cmd)
...
class BLASTDBMaker(CMDTool):
    ...
    def formatDB(self, fasta, out_name, dbtype='nucl', *argv, **kwrgs):
        if os.path.isfile(fasta):
            cmd = self.getCMD(
                " -in {} -dbtype {} -title {}".format(fasta, dbtype, out_name))
            return self.executeCMD(cmd)
        else:
...
class BWA(CMDTool):
    ...
    def alignment(self, algorithm, fastq1, fastq2, genome_fa, out_sam, *argv, **kwrgs):
        cmd = self.getCMD(algorithm, fastq1, fastq2,
                          genome_fa, *argv, **kwrgs)+" > "+out_sam
        self.executeCMD(cmd)
...
class SAMTOOLS(CMDTool):
    ...
    def indexBAM(self, sorted_bam):
        if os.path.isfile(sorted_bam):
            ...
            print("SAMTools(Indexing BAM Error): The bam file ({}) is not exist!".format(
                sorted_bam))
            return -1
    ...
    def indexRefFASTA(self, ref_fasta):
        if os.path.isfile(ref_fasta):
            ...
            print("SAMTools(Index Ref Error): The FASTA file ({}) is not exist!".format(
                ref_fasta))
            return -1
    ...
    def filterBAM(self, bam, bed, sam):
        if os.path.isfile(bam) and os.path.isfile(bed):
            ...
            print("SAMTools(Filter Ref Error): BAM {} or BED {} file are missing!".format(
                bam, bed))
            return -1
    ...
    def trimSE(self, input_fastq, out_fastq, adapter_fa="", phred="-phred33", LEADING=3,
        ...
        if os.path.isfile(input_fastq):
            return self.executeCMD("java -jar "+self.getCMD("SE", phred, '-threads', threads, input_fastq, out_fastq,
                                                            ...
                                                            'TRAILING:{}'.format(
                                                                TRAILING),
                                                            'SLIDINGWINDOW:{}'.format(
                                                                SLIDINGWINDOW),
                                                            'MINLEN:{}'.format(
                                                                MINLEN)
                                                            ))
        else:
            print("Trimmomatic (trimSE Error): input_fastq {} does't exist!".format(
                input_fastq))
            return -1
...
class BWA_SAMTOOLS:
    ...
    def getAlignmentCMD(self, fastq1, fastq2, genome_fa, sorted_bam):
        ...
        if os.path.isfile(fastq1) and os.path.isfile(genome_fa):
            cmd = self.bwa_path + " mem -t " + \
                str(self.p) + " " + genome_fa + " " + fastq1
            if os.path.isfile(fastq2):
                cmd += " " + fastq2
            cmd += " | "
            cmd += self.samtools_path + " sort -@ " + \
                str(self.p)+" -o " + sorted_bam + " - "
        print(cmd)
        return cmd
...
class PICARD:
    ...
    def getMarkDuplicatesCMD(self, bam, filterecd_bam, matrix):
        cmd = "java -jar "+self.path+" MarkDuplicates I=" + \
            bam+" O="+filterecd_bam + " M=" + matrix
...
class BAMCOVERAGE:
    ...
    def getBAM2BiWigCMD(self, bam, bw):
        cmd = self.path+" -b "+bam+" -o "+bw+" -bs " + \
            str(self.binsize) + " -p "+str(self.p)
        return cmd
...
class PLOTHEATMAP:
    def __init__(self, path=""):
        ...
        self.heatmapWidth = 4
        self.plotTitle = ""
        # best,upper-right,upper-left,upper-center,lower-left,lower-right

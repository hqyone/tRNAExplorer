...
def drawExpBoxPlotForGroup(exp_df,sample_df,min_exp_cutof=100, groupby="tRF_First_Type", value="max_exp",fontsize=14, fig_w=12, fig_h =8):
    # Get tRF tables
    group = exp_df.groupby(['tRF_ID.1']).sum()
    #group1= exp_df.groupby('tRF_ID.1').apply(lambda x: ','.join(x.RNA_family))
    group['tRF_Types'] = exp_df.groupby('tRF_ID.1').apply(lambda x: ','.join(x.Type))
    group['tRF_Unique_Types'] = exp_df.groupby('tRF_ID.1').apply(lambda x: ",".join(list(set(x.Type))))
    group['tRF_First_Type'] = exp_df.groupby('tRF_ID.1').apply(lambda x: ",".join(list(set(x.Type))).split(",")[0])
    group['tRNA_families'] = exp_df.groupby('tRF_ID.1').apply(lambda x: ','.join(x.RNA_family))
    group['CODE_AA'] = exp_df.groupby('tRF_ID.1').apply(lambda x: ",".join(list(set(x.CODE_AA))).split(",")[0])
    group['CODE'] = exp_df.groupby('tRF_ID.1').apply(lambda x: ",".join(list(set(x.CODE))).split(",")[0])
    group['AA'] = exp_df.groupby('tRF_ID.1').apply(lambda x: ",".join(list(set(x.AA))).split(",")[0])
    # Filter with min_exp_cutof, above min_exp_cutof means the Gene expressed
    group = group.loc[group.max(axis=1)>min_exp_cutof,]
    group['exp_samples_num'] = group[sample_df["SAMPLE_ID"]].apply(lambda s: (s > 100).sum(), axis=1)
...
# groupby can be "tRF_First_Type","CODE_AA","AA","CODE","RNA_family","CODE_AA"
def drawExpMatrixForFamily(exp_df,sample_df,groupby="RNA_family", fontsize=14, fig_w=12, fig_h =18):
    ...
    group = group[sample_df["SAMPLE_ID"]] # Only left sample data
    # Replace columns name with sample Label
    group.columns = sample_df["SAMPLE_LABEL"]
    # df = normalize(group)
    df = group
    # group = exp_df.groupby(['RNA_family']).sum()
    cm.drawExpressionClusterMatrix(df,'matrix.png',fig_width=fig_w, fig_height=fig_h)

...
def FilterTrfExpMartrixWithSampleIDs(df, sample_ls):
    f_df = df[["tRF_ID","tRNA_Families","RNA_IDs","seq","Type"]]
    sel_s_id_ls=[]
    for s in sample_ls:
        if s in df.columns:
            f_df.loc[:,s] = df[s]
...
def drawExpMatrixForFamily(d,figure="",
                           sample_ls=[],
                           groupby="tRNA_Families",
                           xl_method= "single",
                           yl_method = "centroid",
                           xaxis_font_size=12,
                           yaxis_font_size=12,
    ...
    cm.drawExpressionClusterMatrix(df,figure,
                                   xl_method = xl_method,
                                   yl_method = yl_method,
...
def drawTrfLengthDistribution(d,sample_ls=[],type="read_count"):
    '''
    ...
    sel_s_des = []
    if len(sample_ls) > 1:
        sel_s_ls, exp_df = FilterTrfExpMartrixWithSampleIDs(df, sample_ls)
        if len(sel_s_ls) <= 1:
            exp_df = df
            print("The sample number is "+str(len(sel_s_ls))+", sample filtering abort")
    else:
        sel_s_ls = all_sample_ls
    print("Selected Samples :" + ",".join(sel_s_ls))

    fig, axs = plt.subplots(len(sel_s_ls), 1, figsize=[12, len(sel_s_ls)*1.5 + 2])

    i = 0
    for s in sel_s_ls:
        cur_df = df[["len",s]]
        cur_df = cur_df[cur_df[s]>0]
        sample_des = dl.getSampleLabel(s, sample_df)
        ...
        for d in x:
            sum=0
            if type=="read_count":
                sum = cur_df.loc[cur_df["len"] == d, s].sum()
            else:
                sum = cur_df[cur_df["len"] == d].shape[0]
            y.append(sum)
        axs[i].bar(x, y)
        txt = axs[i].text(0.04, 0.8, sample_des,
                                transform = axs[i].transAxes,
                                va='center', fontsize=10, weight='bold',
                                bbox=dict(facecolor='none', alpha=0.0))
        if i == int(len(sel_s_ls) / 2):
            y_label = "Number of tRFs (type)"
            if type == "read_count":
                y_label = "Read Count"
            axs[i].text(-0.1, 1.3, y_label, transform=axs[i].transAxes,
...
def drawTrfExpMatrixForSample(d,figure="",
    ...
    # group = exp_df.groupby([groupby]).sum()
    group = exp_df.groupby(["tRF_ID"]).sum()
    group = group[group.max(axis=1) / len(sel_s_des) > min_int]
    group = np.log10(group+1)
    group = group[sel_s_ls] # Only left sample data
    # Replace columns name with sample Label
    group.columns = sel_s_des
    ...
    # group = exp_df.groupby(['RNA_family']).sum()
    if figure.strip() == "":
        figure = d["report_dir"]+"trf_exp_matrix.png"
    cm.drawExpressionClusterMatrix(df, figure,
                                   xl_method=xl_method,
                                   yl_method=yl_method,
                                   xaxis_font_size=xaxis_font_size,

...
def drawTrfDistForSamples(d, fontsize):
    ...
    for t in types:
        ...
        if t in df:
            used_types.append(t)
            axs[0].bar(df["#SampleID"], df[t].values, width, bottom=buttom1, color=types_colors[t], edgecolor='k' )
            axs[1].bar(ratio_df["#SampleID"], ratio_df[t].values, width, bottom=buttom2, color=types_colors[t], edgecolor='k')
            index+=1
...
def drawTrfDistForAcceptorsInSamples(d, font_size=18, test=False):
    '''
    ...
    exp_df = d["exp_df"]
    types = d["types"]
    types_color_map =d["types_colors"]
    sample_ls = d["sample_ls"]
    sample_df = d["s_df"]
    ...
    for s_id in sample_ls:
        ...
        for t in exist_types:
            ...
            axs[0].bar(aa_group.index.tolist(),
                       plot_type_aa_group.loc[(t)].reindex(aa_group.index.tolist()).Value, width, bottom=buttom1, color=types_color_map[t],edgecolor='k')
            axs[1].bar(aa_group.index.tolist(),
                       plot_type_aa_group.loc[(t)].reindex(aa_group.index.tolist()).Value/aa_group['Value'], width, bottom=buttom2, color=types_color_map[t],edgecolor='k')
            index+=1
        ...
        #axs[0].get_xaxis().set_ticks([])
        axs[0].set_ylabel("Normalized Reads (K)", fontsize=font_size-1)
        #https://pythonspot.com/matplotlib-legend/
        axs[1].legend(exist_types,loc='upper center', bbox_to_anchor=(1.15, 1.02), ncol=1)
        axs[1].set_ylabel("Ratio", fontsize=font_size-2)
        axs[1].set_xlabel("Amino acid", fontsize=font_size-1)
        ...
        axs[0].text(0.95, 0.90, s_label, transform=axs[0].transAxes,
                                                  horizontalalignment='right', fontsize=font_size, weight='bold')
        #plt.show()
        fig.tight_layout()
        if test:
            plt.show()
